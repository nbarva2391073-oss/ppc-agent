...
#
# Оновлення 23.09.2026 (Round 2, за результатами обговорення з Nataly):
#   - EVIDENCE в Round 1 тепер завжди починається з дати/періоду даних —
#     щоб на Round 2 відрізняти реальну розбіжність висновків від
#     розбіжності через різні періоди аналізу.
#   - POSITION в Round 2 має 4-й варіант "Unclear" — коли висновок іншої
#     сторони справді не піддається чесній класифікації (розмитий текст
#     або надто різні періоди даних). Ніякого автоматичного 3-го раунду
#     це не запускає — рішення продовжувати чи ні завжди за Nataly.
#
# Запуск:
...
EVIDENCE: ПЕРШИМ РЯДКОМ обов'язково вкажи дату/період даних, на яких
базується висновок (наприклад "Дані станом на 21.09.2026, період
01.09-21.09.2026"), а потім ключові цифри/факти. Це потрібно, щоб на
Round 2 можна було відрізнити реальну розбіжність висновків від
розбіжності через різні періоди даних.
...
ПЕРЕД тим як визначити POSITION — звір, чи обидві сторони аналізували
той самий період даних (перший рядок EVIDENCE у кожної сторони). Якщо
періоди істотно різняться — це найімовірніша причина розбіжності, а
не різна логіка чи різне бачення. У такому разі прямо зазнач це в
REASON ("різні періоди даних, а не змістова незгода"), а не мовчки
пиши "Disagree".

ФОРМАТ ВІДПОВІДІ (обов'язково саме такими маркерами):

POSITION: Agree / Partially agree / Disagree / Unclear.
(Unclear — тільки якщо висновок ChatGPT справді неможливо чесно
класифікувати: немає чіткої позиції, текст занадто розмитий, або
дані настільки різні за періодом, що порівняння втрачає сенс. Не
використовуй "Unclear" замість "Disagree", якщо позиція насправді
просто інша.)
REASON: чому саме така позиція — конкретно, з посиланням на докази.
Якщо POSITION = Unclear, вкажи, яка саме фраза чи частина висновку
ChatGPT незрозуміла або через що саме порівняння неможливе.
